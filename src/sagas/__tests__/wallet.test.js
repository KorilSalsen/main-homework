...
describe('wallet saga', () => {
  ...
  describe('watch', () => {
    ...
    it('1. Effect take loginSuccess, logout, walletRequest', () => {
      expect(saga.next().value).toEqual(take([loginSuccess, logout, walletRequest]));
    });
    ...
    it('3. Effect take loginSuccess, logout, walletRequest', () => {
      expect(saga.next(walletTask).value).toEqual(take([loginSuccess, logout, walletRequest]));
    });
  ...
  describe('flow', () => {
    ...
    it('2. Effect put walletSuccess', () => {
      expect(saga.next(response).value).toEqual(put(walletSuccess(response.data.result)));
    });
  });
