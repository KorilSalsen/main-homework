...
describe('user saga', () => {
  ...
  describe('watch', () => {
    ...
    it('1. Effect take loginSuccess, logout, userRequest', () => {
      expect(saga.next().value).toEqual(take([loginSuccess, logout, userRequest]));
    });
    ...
    it('3. Effect take loginSuccess, logout, userRequest', () => {
      expect(saga.next(userTask).value).toEqual(take([loginSuccess, logout, userRequest]));
    });
  ...
  describe('flow', () => {
    ...
    it('2. Effect put userSuccess', () => {
      expect(saga.next(response).value).toEqual(put(userSuccess(response.data.result)));
    });
  });
